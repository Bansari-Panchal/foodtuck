 ...
 def self.find_for_facebook_oauth(auth)
    ...
    unless user.nil?
        ...
        unless registered_user.nil?
            ...
            user = User.create!(
            name: auth.info.name,email: auth.info.email,password: Devise.friendly_token[0,20],)
            ser_provider = UserProvider.create!(
            provider:auth.provider,uid:auth.uid,user_id: user.id)
            user
        end
    end
  end
